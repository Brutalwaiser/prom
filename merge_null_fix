...
#     echo $ID_QRES
  ...
  if [ $STATUS_QRES == "1" ] && [ $PROC_QRES == "1" ]
  then
      echo "Нашли строку с открытым трапом. Начали клирить"
             TRAP="NRT_MERGE_EVENT;NRT_MRGPROC_NULL;CLEAR;Отсутствуют запущенные процессы merge manager"
             SUBJECT="Отстутствие входного потока данных для merge"
             MSGEMAIL="Проблема устранена! Появился активный процесс merge manager."
          ...
          CLEAR_QRY="UPDATE pmdata.Incident_Log_Table
                     ...
                     WHERE incident_id=(select max(incident_id) from pmdata.Incident_Log_Table where operation_id = 1);"

          CLEARING=$(echo $CLEAR_QRY | sqlplus -S "$USER"/"$PASSWORD"@"$SERVICE")
          echo $CLEARING
   else
       echo "Трапов нет, клирить нечего"
